...
electron_1.ipcMain.handle('select-download-folder', async () => {
...
function downloadPdfFile(url, destinationPath, paperId, redirectCount = 0) {
...
// PDF herunterladen
electron_1.ipcMain.handle('download-pdf', async (event, args) => {
...
electron_1.ipcMain.handle('write-file', async (event, args) => {
...
// Ordner erstellen
electron_1.ipcMain.handle('create-directory', async (event, dirPath) => {
    try {
...
// DevTools Toggle
electron_1.ipcMain.handle('toggle-devtools', async () => {
    if (mainWindow) {
